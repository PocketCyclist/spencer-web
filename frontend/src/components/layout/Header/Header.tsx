...
export const Header = () => {
  ...
  useEffect(() => {
    document.documentElement.classList[isMenuOpen ? 'add' : 'remove']('overflow-hidden');
  }, [isMenuOpen]);
  ...
  return (
    <header className="w-full h-mobile-header fixed top-0 left-0 z-10 bg-background lg:h-header">
      <div className="container h-full flex justify-between items-center gap-y-4">
        <div>
          <Image
            className="w-[152px] h-[46px] lg:w-[264px] lg:h-[79px]"
        ...
        >
          <ul className="space-y-10 lg:flex lg:items-center lg:space-x-8 lg:space-y-0">
            {ITEMS.map((item, index) => (
              <li key={index}>
                <a aria-current={pathname === item.url ? 'page' : undefined} className={cn(pathname === item.url && 'relative pointer-events-none')} href={item.url} title={item.title}>
                  {item.isActive && <CapaIcon className="absolute -left-2 top-1/2 text-yellow -translate-y-1/2 lg:-left-[1.125rem]" />}
